...
void main() {
    ...
    vec3 result = frag.rgb * weight[0] / modi; // current fragment's contribution
    
    if(u_horizontal)
    {
        for(int i = 1; i < 5; ++i)
        {
            frag = texture(u_texture, v_uv + vec2(tex_offset.x * float(i), 0.0));
            result += frag.rgb * weight[i] / modi;
            frag = texture(u_texture, v_uv - vec2(tex_offset.x * float(i), 0.0));
            result += frag.rgb * weight[i] / modi;
        }
    }
